...
import {
  PHONES, BRANDS, SERVICES, ACCESSORIES,
  WHATSAPP_NUMBER, DISPLAY_NUMBER, fmt
} from "@/data/products";
      ...
      <section id="phones" className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:py-20">
        <div className="mb-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {filtered.length} model{filtered.length !== 1 ? "s" : ""} available · Prices updated regularly
            </p>
          </div>
          <a
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-4 py-24 text-center text-muted-foreground">
            ...
            <button onClick={() => { setBrand("All"); setPriceRange(0); setSearch(""); setSort("default"); }} className="rounded-full border border-border px-5 py-2 text-sm text-foreground hover:bg-secondary transition">
              Clear Filters
            </button>
          </div>
        ) : (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {SERVICES.map((s) => (
              ...
              >
                ...
                <div className="mt-auto font-display text-sm font-bold text-primary">{s.price}</div>
              </a>
            ))}
          </div>
